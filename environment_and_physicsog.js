...
// Rewritten to fix vector wall 3D pivot points.
    ...
    createWalls(items) {
        for (const [key, item] of items) {
            ...
            if (key.startsWith('VEC_')) {
                ...
                // Translate the geometry so its origin (pivot) is at the start of the wall
                wallGeo.translate(length / 2, 0, 0);
                
                mesh = new THREE.Mesh(wallGeo, material);

                // Position the mesh (with its now-offset pivot) at the starting vertex
                mesh.position.set(
