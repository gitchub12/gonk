...
// Rewritten to fix bugs and add multi-tile placement feature.
...
class LevelEditor {
    constructor() {
        ...
        this.activeTileSize = 1; // 1, 2, or 4
    ...
    eraseItem(gridX, gridY, worldX, worldY) {
        ...
        const action = () => {
            ...
            if (!this.lineLayers.includes(this.activeLayerName)) {
                // For tile layers, find what to erase, considering multi-tiles
    ...
    placeItem(gridX, gridY) {
        ...
        this.modifyState(() => {
            ...
            // Clear area for multi-tile placement
            if (size > 1) {
    ...
    handlePaintAction(e) {
        ...
        } else { // NPC, Asset, Spawn layers
            this.activeTileSize = 1; // Force 1x1
            this.placeItem(gridX, gridY);
        }
    }
    ...
    onMouseDown(e) {
        ...
        this.lastMouse = { x: e.clientX, y: e.clientY }; // Always update lastMouse
    ...
    onMouseMove(e) {
        ...
        this.lastMouse = { x: e.clientX, y: e.clientY }; // Keep updating for ghost line
    ...
    render() { /* ... (logic from previous version largely fine, but needs renderTileLayer update) ... */ this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height); this.ctx.save(); this.ctx.translate(this.panX, this.panY); this.ctx.scale(this.zoom, this.zoom); const gs = this.gridSize; const activeLayerIndex = this.layerOrder.indexOf(this.activeLayerName); const visStartX = Math.floor(-this.panX / this.zoom / gs); const visEndX = Math.ceil((this.canvas.width - this.panX) / this.zoom / gs); const visStartY = Math.floor(-this.panY / this.zoom / gs); const visEndY = Math.ceil((this.canvas.height - this.panY) / this.zoom / gs); const startX = Math.max(0, visStartX); const endX = Math.min(this.gridWidth, visEndX); const startY = Math.max(0, visStartY); const endY = Math.min(this.gridHeight, visEndY); for (let i = 0; i < this.layerOrder.length; i++) { const layerName = this.layerOrder[i]; const isLineLayer = this.lineLayers.includes(layerName); const isTransparent = i > activeLayerIndex || (isLineLayer && layerName !== this.activeLayerName); if (isTransparent) this.ctx.globalAlpha = 0.35; if (isLineLayer) this.renderWallLayer(this.levelData[layerName]); else this.renderTileLayer(layerName, this.levelData[layerName], startX, startY, endX, endY); if (isTransparent) this.ctx.globalAlpha = 1.0; } this.drawGridAndBorders(startX, startY, endX, endY); this.drawHoverAndVectorUI(visStartX, visStartY, visEndX, visEndY); this.ctx.restore(); }
    ...
    renderGridWall(key, item, gs, wallThickness) { /* ... (unchanged) ... */ const [type, xStr, zStr] = key.split('_'); const x = Number(xStr); const z = Number(zStr); const img = this.preloadedImages.get(item.key); if (!img) { this.ctx.strokeStyle = item.key.includes('/door/') ? 'cyan' : 'white'; this.ctx.lineWidth = 4 / this.zoom; this.ctx.beginPath(); if (type === 'V') { this.ctx.moveTo((x + 1) * gs, z * gs); this.ctx.lineTo((x + 1) * gs, (z + 1) * gs); } else { this.ctx.moveTo(x * gs, (z + 1) * gs); this.ctx.lineTo((x + 1) * gs, (z + 1) * gs); } this.ctx.stroke(); return; } this.ctx.save(); const pattern = this.ctx.createPattern(img, 'repeat'); this.ctx.fillStyle = pattern; if (type === 'V') { this.ctx.translate((x + 1) * gs, z * gs); this.ctx.fillRect(-wallThickness / 2, 0, wallThickness, gs); } else { this.ctx.translate(x * gs, (z + 1) * gs); this.ctx.fillRect(0, -wallThickness / 2, gs, wallThickness); } this.ctx.restore(); }
    renderVectorWall(item) { /* ... (unchanged) ... */ const [x1, y1, x2, y2] = item.points.map(p => p * this.gridSize); const img = this.preloadedImages.get(item.key); const wallThickness = Math.max(4, this.gridSize * 0.15) / this.zoom; const dx = x2 - x1, dy = y2 - y1; const length = Math.sqrt(dx * dx + dy * dy); const angle = Math.atan2(dy, dx); this.ctx.save(); this.ctx.translate(x1, y1); this.ctx.rotate(angle); if (img) { const pattern = this.ctx.createPattern(img, 'repeat'); this.ctx.fillStyle = pattern; } else { this.ctx.fillStyle = '#FFF'; } this.ctx.fillRect(0, -wallThickness / 2, length, wallThickness); this.ctx.restore(); }
    ...
    async saveLevel() { const levelObject = { settings: { width: this.gridWidth, height: this.gridHeight, defaults: this.defaultTextures }, layers: {} }; for (const layerName of this.layerOrder) { const layerMap = this.levelData[layerName]; if (layerMap.size > 0) levelObject.layers[layerName] = Array.from(layerMap.entries()); } const jsonString = JSON.stringify(levelObject, null, 2); const blob = new Blob([jsonString], { type: 'application/json' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `level_${this.currentLevel}.json`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(a.href); this.isLevelDirty = false; this.history = [this.cloneLevelData(this.levelData)]; this.historyIndex = 0; this.statusMsg.textContent = `Level ${this.currentLevel} saved.`; }
